...
import_search()
{
  ...
  do
    ...
    JSON=`echo "$LINES" | jq -Rsn '{"list": [inputs | . / "\r\n" | (.[] | select(length > 0) | . / "	") as $input | {"pk": $input[0], "personId": $input[2], "searchTypeNum": $input[3], "searchVehicle": ($input[4] == "1"), "searchDriver": ($input[5] == "1"), "searchPassenger": ($input[6] == "1"), "searchProperty": ($input[7] == "1"), "searchVehicleSiezed": ($input[8] == "1"), "searchPersonalPropertySiezed": ($input[9] == "1"), "searchOtherPropertySiezed": ($input[10] == "1")}]}'`
