...
public class ClusterEnUSGenApiServiceImpl implements ClusterEnUSGenApiService {
		...
		try {
			...
			if(facetPivot != null) {
				...
				while(facetPivotIterator.hasNext()) {
					...
					responsePivotSearchCluster(pivotFields, pivotArray);
				}
			}
	...
	public void responsePivotSearchCluster(List<PivotField> pivotFields, JsonArray pivotArray) {
	...
	public void createCluster(SiteRequestEnUS siteRequest, Handler<AsyncResult<Cluster>> eventHandler) {
		try {
			...
			tx.preparedQuery("INSERT INTO Cluster(created, userKey) VALUES($1, $2) RETURNING pk")
	...
	public void defineCluster(Cluster o, Handler<AsyncResult<OperationResponse>> eventHandler) {
		try {
			...
			Transaction tx = siteRequest.getTx();
			Long pk = o.getPk();
			tx.preparedQuery("SELECT * FROM Cluster WHERE pk=$1")
					.collecting(Collectors.toList())
					.execute(Tuple.of(pk)
					, defineAsync
			-> {
