...
public class TrafficSearchEnUSGenApiServiceImpl extends BaseApiServiceImpl implements TrafficSearchEnUSGenApiService {
	...
	public void postTrafficSearchFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest).onSuccess(siteRequest -> {
			...
			postTrafficSearchFuture(siteRequest, false).onSuccess(o -> {
			...
			num++;
				futures2.add(0, Future.future(a -> {
					sqlConnection.preparedQuery(bSql.toString())
						...
						RuntimeException ex2 = new RuntimeException("value TrafficSearch.searchOtherPropertySiezed failed", ex);
						LOG.error(String.format("attributeTrafficSearch failed. "), ex2);
					...
					searchTrafficSearchList(siteRequest, false, true, true, "/api/traffic-search", "PATCH").onSuccess(listTrafficSearch -> {
						try {
							...
							if(listTrafficSearch.getQueryResponse().getResults().getNumFound() > 1
								...
								apiRequest.setPk(Optional.ofNullable(listTrafficSearch.first()).map(o2 -> o2.getPk()).orElse(null));
								eventBus.publish("websocketTrafficSearch", JsonObject.mapFrom(apiRequest).toString());
			...
			try {
				...
				searchTrafficSearchList(siteRequest, false, true, true, "/api/traffic-search", "PATCH").onSuccess(listTrafficSearch -> {
					try {
						...
						if(o != null && listTrafficSearch.getQueryResponse().getResults().getNumFound() == 1) {
							ApiRequest apiRequest = new ApiRequest();
							apiRequest.setRows(1);
							apiRequest.setNumFound(1L);
							apiRequest.setNumPATCH(0L);
	...
	public Future<TrafficSearch> createTrafficSearch(SiteRequestEnUS siteRequest) {
		...
		try {
			...
			sqlConnection.preparedQuery("INSERT INTO TrafficSearch(created) VALUES($1) RETURNING pk")
				...
				LOG.error("createTrafficSearch failed. ", ex2);
				promise.fail(ex2);
			});
		} catch(Exception ex) {
	...
	public String searchTrafficSearchFq(String uri, String apiMethod, SearchList<TrafficSearch> searchList, String entityVar, String valueIndexed, String varIndexed) {
		...
		if(StringUtils.startsWith(valueIndexed, "[")) {
			...
			return varIndexed + ":" + ClientUtils.escapeQueryChars(TrafficSearch.staticSolrFqForClass(entityVar, searchList.getSiteRequest_(), valueIndexed)).replace("\\", "\\\\");
		}
	}
	...
	public Future<Void> defineTrafficSearch(TrafficSearch o) {
		...
		try {
			...
			sqlConnection.preparedQuery("SELECT * FROM TrafficSearch WHERE pk=$1")
				...
				RuntimeException ex2 = new RuntimeException(ex);
				LOG.error(String.format("defineTrafficSearch failed. "), ex2);
				promise.fail(ex2);
			});
		} catch(Exception ex) {
			LOG.error(String.format("defineTrafficSearch failed. "), ex);
		...
		try {
			...
			if(refresh && !Optional.ofNullable(siteRequest.getJsonObject()).map(JsonObject::isEmpty).orElse(true)) {
				...
				CompositeFuture.all(futures).onSuccess(b -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", Optional.ofNullable(siteRequest.getUser()).map(user -> user.principal()).orElse(null));
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("opendatapolicing-enUS-TrafficSearch", json, new DeliveryOptions().addHeader("action", "patchTrafficSearchFuture")).onSuccess(c -> {
