...
public class PageDesignGenPage extends PageDesignGenPageGen<PageLayout> {
	...
	public void tbody2PageDesignGenPage() {
		...
		for(int i = 0; i < listPageDesign.size(); i++) {
			...
			String uri = "/page-design/" + o.getnull();
			{ e("tr").f();
			} g("tr");
		}
	}
			...
			if(listPageDesign != null && listPageDesign.size() == 1) {
				{ e("button")
					.a("class", "w3-btn w3-round w3-border w3-border-black w3-ripple w3-padding w3-khaki ")
						...
						.a("onclick", "patchPageDesignVals( [ {name: 'fq', value: ':' + " + siteRequest_.getRequest() + " } ], {}, function() { addGlow($('#refreshThisPageDesignGenPage')); }, function() { addError($('#refreshThisPageDesignGenPage')); }); return false; ").f();
